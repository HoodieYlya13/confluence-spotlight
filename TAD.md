...
`bun run tauri build` produces a `.app` + `.dmg` under `src-tauri/target/release/bundle/`; the `.dmg` is the downloadable artifact. The build is intentionally **unsigned/un-notarized** (this is a demo), so first launch needs the Gatekeeper bypass — right-click → Open, or `xattr -dr com.apple.quarantine <app>`. Because the shipped binary holds no secrets, the only build-time configuration is the two public URLs: `DEFAULT_SERVER_URL` and `DEFAULT_AUTH_URL` in `mcp.rs` (overridable at build via `MCP_SERVER_URL` / `SPOTLIGHT_AUTH_URL`). `DEFAULT_AUTH_URL` **must** point at the deployed console. A GitHub Actions `tauri-apps/tauri-action` workflow can attach the `.dmg` to a Release for download.
